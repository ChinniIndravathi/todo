        ...
        this.todoTasks.forEach((todoTask: TodoTask)=>{
            this.todoItemsHTML.innerHTML += `
                <li class="${todoTask.completed? 'completed': 'notcompleted'}">
                    ${todoTask.task}
                    <button id="done-button" class="fas fa-check" onclick="todoApp.markCompleted(${todoTask.id})"></button>
                    <button id="delete-icon" class="fas fa-trash-alt delete-icon" onclick="todoApp.removeTask(${todoTask.id})"></button>
                </li>
            `
        })
    }
